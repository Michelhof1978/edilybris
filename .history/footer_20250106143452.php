...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (isMobile) {
        ...
        function updateGroupVisibility() {
            groups.forEach((group, index) => {
                if (index === currentGroupIndex) {
                    group.classList.remove("new-hidden");  // Affiche le groupe actuel
                } else {
                    group.classList.add("new-hidden");     // Cache les autres groupes
                }
            });

            // Mettre à jour l'état des boutons
            backButton.disabled = currentGroupIndex === 0;  // Le bouton Précédent est désactivé sur la première carte
            nextButton.disabled = currentGroupIndex === groups.length - 1;  // Le bouton Suivant est désactivé sur la dernière carte
        }

        // Gestionnaire de clic pour le bouton "Suivant"
        nextButton.addEventListener("click", function () {
            if (currentGroupIndex < groups.length - 1) {  // Vérifie si on n'est pas déjà sur la dernière carte
                currentGroupIndex++;
        ...
        backButton.addEventListener("click", function () {
            if (currentGroupIndex > 0) {  // Vérifie si on n'est pas déjà sur la première carte
                currentGroupIndex--;
                updateGroupVisibility();
            }
        });
        ...
        // Initialisation
        updateGroupVisibility(); // Affiche la première carte au début
    }
});
</script>
