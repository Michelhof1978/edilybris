...
<div class="container">
    ...
    <img src="images/logoHarmonyDigital.png" alt="Logo Harmony Digital" class="img-fluid w-25 logoHarmony">
                    ...
                    <div class="copyright-text">
                        <p>
                            Copyright &copy; 2010 - <span id="date2"></span> Droits réservés <a href="https://harmony-digital.fr/" target="_blank"><br>
                        </p>
                    </div>
                </div>
            </div>
...
<script>
 document.addEventListener("DOMContentLoaded", function () {
    let currentMobileGroup = 1; // Groupe initial (mobile)
    let currentDesktopGroup = 1; // Groupe initial (desktop)
    const totalMobileGroups = 10; // Total des groupes mobiles
    const totalDesktopGroups = 4; // Total des groupes desktop

    // Boutons mobiles
    const mobileBackButton = document.getElementById("new-mobile-backButtonMobile");
    const mobileNextButton = document.getElementById("new-mobile-nextGroupButtonMobile");

    // Boutons desktop
    const desktopBackButton = document.getElementById("new-desktop-backButtonDesktop");
    const desktopNextButton = document.getElementById("new-desktop-nextGroupButtonDesktop");

    function updateMobileGroups() {
        // Cacher tous les groupes
        for (let i = 1; i <= totalMobileGroups; i++) {
        ...
        const currentGroup = document.getElementById(`new-group${currentMobileGroup}`);
        if (currentGroup) {
            currentGroup.classList.remove("new-hidden");
        }

        // Gérer les états des boutons
        mobileBackButton.disabled = currentMobileGroup === 1;
        mobileNextButton.disabled = currentMobileGroup === totalMobileGroups;
    }

    function updateDesktopGroups() {
        // Cacher tous les groupes
        for (let i = 1; i <= totalDesktopGroups; i++) {
            const group = document.getElementById(`desktop-group${i}`);
            if (group) {
                group.classList.add("new-hidden");
            }
        }
        // Afficher le groupe actuel
        const currentGroup = document.getElementById(`desktop-group${currentDesktopGroup}`);
        if (currentGroup) {
            currentGroup.classList.remove("new-hidden");
        }

        // Gérer les états des boutons
        desktopBackButton.disabled = currentDesktopGroup === 1;
        desktopNextButton.disabled = currentDesktopGroup === totalDesktopGroups;
    }

    // Écouteurs pour les boutons mobiles
    mobileBackButton.addEventListener("click", function () {
        if (currentMobileGroup > 1) {
            currentMobileGroup--;
            updateMobileGroups();
        }
    });
    ...
    mobileNextButton.addEventListener("click", function () {
        if (currentMobileGroup < totalMobileGroups) {
            currentMobileGroup++;
            updateMobileGroups();
        }
    });

    // Écouteurs pour les boutons desktop
