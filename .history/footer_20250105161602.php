...
<script>
        document.addEventListener('DOMContentLoaded', function () {
            // Variables pour les boutons mobile et desktop
            ...
            // Ajout des groupes pour mobile
            const mobileGroups = Array.from({ length: 10 }, (_, i) =>
                document.getElementById(`mobile-group${i + 1}`)
            );

            // Ajout des groupes pour desktop
            const desktopGroups = Array.from({ length: 10 }, (_, i) =>
            ...
            function hideAllGroups(groups) {
                groups.forEach(group => {
                    if (group) {
                        group.classList.add('mobile-hidden', 'new-hidden');
                    }
                });
            }
            ...
            function showGroup(groups, index) {
                if (groups[index]) {
                    groups[index].classList.remove('mobile-hidden', 'new-hidden');
                }
            }
            ...
            function updateMobileButtons() {
                ...
                if (currentMobileGroup === 0) {
                    ...
                    mobileBackBtn.classList.add('disabled-btn');
                } else {
                    mobileBackBtn.disabled = false;
                    mobileBackBtn.classList.remove('disabled-btn');
                }

                // Bouton "Suivant"
                if (currentMobileGroup === mobileGroups.length - ) {
                    mobileNextBtn.disabled = true;
                    mobileNextBtn.classList.add('disabled-btn');  // Ajout de la classe pour le gris
                } else {
                    mobileNextBtn.disabled = false;
                    mobileNextBtn.classList.remove('disabled-btn');  // Retirer la classe
                }
            }
            ...
            mobileNextBtn.addEventListener('click', function () {
                if (currentMobileGroup < mobileGroups.length - 3) {
                    hideAllGroups(mobileGroups);
                    currentMobileGroup++;
                    showGroup(mobileGroups, currentMobileGroup);
                    updateMobileButtons();
