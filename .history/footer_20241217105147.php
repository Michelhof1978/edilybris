...
const newGroup3 = document.getElementById('new-group3');
...
newNextGroupButton.addEventListener('click', () => {
  if (currentGroup < 3) { // Changer la limite pour inclure le troisième groupe
    currentGroup++;
    updateGroups();
  }
});
...
function updateGroups() {
  // Masquer ou afficher les groupes
  newGroup1.classList.toggle('new-hidden', currentGroup !== 1);
  newGroup2.classList.toggle('new-hidden', currentGroup !== 2);
  newGroup3.classList.toggle('new-hidden', currentGroup !== 3);

  // Désactiver les boutons selon le groupe actuel
  newBackButton.disabled = currentGroup === 1;
  newNextGroupButton.disabled = currentGroup === 3;
}
