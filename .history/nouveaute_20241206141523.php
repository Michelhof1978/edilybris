...
document.getElementById('L-nextGroupButton').addEventListener('click', function() {
    var currentGroup = document.querySelector('.L-card-container:not(.d-none)');
    var nextGroup = currentGroup.nextElementSibling;

    if (nextGroup && nextGroup.classList.contains('L-card-container')) {
        currentGroup.classList.add('d-none');
        nextGroup.classList.remove('d-none');
    }
    
    // Désactiver le bouton précédent si on est sur le premier groupe
    document.getElementById('L-backButton').disabled = false;
    if (!nextGroup || !nextGroup.nextElementSibling) {
        document.getElementById('L-nextGroupButton').disabled = true;
    }
});

document.getElementById('L-backButton').addEventListener('click', function() {
    var currentGroup = document.querySelector('.L-card-container:not(.d-none)');
    var prevGroup = currentGroup.previousElementSibling;

    if (prevGroup && prevGroup.classList.contains('L-card-container')) {
        currentGroup.classList.add('d-none');
        prevGroup.classList.remove('d-none');
    }

    // Désactiver le bouton suivant si on est sur le dernier groupe
    document.getElementById('L-nextGroupButton').disabled = false;
    if (!prevGroup || !prevGroup.previousElementSibling) {
        document.getElementById('L-backButton').disabled = true;
    }
});
