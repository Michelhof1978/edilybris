...
<script>
// Mobile Navigation
const mobileGroups = document.querySelectorAll('.mobile-card-group');
...
mobileNextButton.addEventListener('click', () => {
    if (currentMobileIndex < mobileGroups.length - 1) {
        mobileGroups[currentMobileIndex].classList.add('hidden');
        currentMobileIndex++;
        mobileGroups[currentMobileIndex].classList.remove('hidden');
        updateMobileButtons();
    }
});
...
// Desktop Navigation
const desktopGroups = document.querySelectorAll('.desktop-card-group');
let currentDesktopIndex = 0;
const desktopBackButton = document.getElementById('desktop-back-button');
const desktopNextButton = document.getElementById('desktop-next-button');

desktopNextButton.addEventListener('click', () => {
    if (currentDesktopIndex < desktopGroups.length - 1) {
        desktopGroups[currentDesktopIndex].classList.add('hidden');
        currentDesktopIndex++;
        desktopGroups[currentDesktopIndex].classList.remove('hidden');
        updateDesktopButtons();
    }
});
...
function updateDesktopButtons() {
    ...
    desktopNextButton.disabled = currentDesktopIndex === desktopGroups.length - 1; // Corrected this line
}
