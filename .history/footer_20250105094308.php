...
document.addEventListener("DOMContentLoaded", function () {
  let currentGroup = 1;
  const totalGroups = 12; // Ajustez en fonction du nombre total de groupes
  const backButton = document.getElementById("mobile-backButton");
  const nextButton = document.getElementById("mobile-nextButton");

  function updateGroups() {
    // Masquer tous les groupes
    document.querySelectorAll(".mobile-card-container").forEach(group => {
      group.classList.add("mobile-hidden");
    });

    // Afficher le groupe actuel
    const currentGroupElement = document.getElementById(`mobile-group${currentGroup}`);
    if (currentGroupElement) {
      currentGroupElement.classList.remove("mobile-hidden");
    }

    // Activer ou désactiver les boutons
    backButton.disabled = currentGroup === 1;
