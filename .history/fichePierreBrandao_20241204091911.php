...
<main>
    ...
    <div class="containerAuteur">
        ...
        <!-- Affichage des livres pour la page actuelle -->
        <?php foreach ($livresPage as $livre): ?>
            <div class="livre">
                <div class="livre-content">
                    <div class="livre-image">
                        <img src="<?= $livre['image'] ?>" class="img-fluid w-75" alt="Couverture du livre">
                        <p class="fw-bold coloryellow mt-3 fs-4"><?= $livre['prix'] ?></p>
                    </div>
                    <div class="livre-details">
                        <h3 class="h2Jaune"><?= $livre['titre'] ?></h3>
                        <p class="annee text-white"><strong class="h2Jaune">Année de publication :</strong> <?= $livre['annee'] ?></p>
                        <p><strong class="h2Jaune">Résumé :</strong> <?= $livre['resume'] ?></p>
                        <a href="<?= $livre['lien'] ?>" class="lien text-white fw-bold" download>Télécharger le bon de commande</a>
                    </div>
                </div>
            </div>
        <?php endforeach; ?>
        ...
        <div class="livre-navigation d-flex justify-content-center mt-4">
            <!-- Bouton Précédent -->
            <button class="livre-button" <?= $page <= 1 ? 'disabled' : '' ?> onclick="window.location.href='?page=<?= $page - 1 ?>'">Précédent</button>

            <!-- Bouton Suivant -->
            <button class="livre-button" <?= $page >= $totalPages ? 'disabled' : '' ?> onclick="window.location.href='?page=<?= $page + 1 ?>'">Suivant</button>
        </div>
    </div>
</section>
