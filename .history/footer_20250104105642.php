...
document.addEventListener("DOMContentLoaded", function() {
  ...
  const mobileConfig = {
    currentGroup: 1,
    totalGroups: 10,
    nextButtonId: 'new-nextGroupButton',
    backButtonId: 'new-backButton',
    groupPrefix: 'new-group',
  ...
  const desktopConfig = {
    currentGroup: 1,
    totalGroups: 4,
    nextButtonId: 'L-nextGroupButton2',
    backButtonId: 'L-backButton2',
    groupPrefix: 'L-group',
    hiddenClass: 'L-hidden',
  };

  // Fonction générale pour gérer les groupes
  const setupNavigation = ({ currentGroup, totalGroups, nextButtonId, backButtonId, groupPrefix, hiddenClass }) => {
    ...
    const showGroup = (groupNumber) => {
      ...
      for (let i = 1; i <= totalGroups; i++) {
        const group = document.getElementById(`${groupPrefix}${i}`);
        if (group) {
      ...
      const current = document.getElementById(`${groupPrefix}${groupNumber}`);
      if (current) {
        current.classList.remove(hiddenClass);
      }
  ...
  // Initialiser la navigation pour mobile et desktop
  setupNavigation(mobileConfig);
  setupNavigation(desktopConfig);
});
</script>
