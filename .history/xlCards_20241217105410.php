<!-- CARDS XL Mobile -->
<div class="d-block d-md-none">
  <!-- Carte 1 -->
  <div id="new-group1" class="new-card-container">
    <div class="new-card">
      <a href="link_to_book_A.html">
        <img src="images/pierreBrandao/XLlamuserie.webp" class="img-fluid" alt="Couverture de livre l'amour a fleur d'âme">
        <div class="new-card-content">
          <div class="new-card-title">L'écho des collines</div>
          <div class="new-card-author">Alice Dubois</div>
          <div class="new-card-price">16,99 €</div>
        </div>
      </a>
    </div>
  </div>
  
  <!-- Carte 2 -->
  <div id="new-group2" class="new-card-container new-hidden">
    <div class="new-card">
      <a href="link_to_book_B.html">
        <img src="images/pierreBrandao/XLlamuserie.webp" class="img-fluid" alt="Couverture de livre">
        <div class="new-card-content">
          <div class="new-card-title">Les murmures du temps</div>
          <div class="new-card-author">Bertrand Martin</div>
          <div class="new-card-price">18,99 €</div>
        </div>
      </a>
    </div>
  </div>

  <!-- Carte 3 -->
  <div id="new-group3" class="new-card-container new-hidden">
    <div class="new-card">
      <a href="link_to_book_C.html">
        <img src="images/pierreBrandao/XLquesavonsnousdumonde.png" class="img-fluid" alt="Couverture de livre">
        <div class="new-card-content">
          <div class="new-card-title">Le cercle des étoiles</div>
          <div class="new-card-author">Camille Laurent</div>
          <div class="new-card-price">12,99 €</div>
        </div>
      </a>
    </div>
  </div>

  <!-- Carte 4 -->
  <div id="new-group4" class="new-card-container new-hidden">
    <div class="new-card">
      <a href="link_to_book_D.html">
        <img src="images/pierreBrandao/XLlamourafleurdame.png" class="img-fluid" alt="Couverture de livre">
        <div class="new-card-content">
          <div class="new-card-title">Sous les cyprès</div>
          <div class="new-card-author">Diane Morel</div>
          <div class="new-card-price">14,99 €</div>
        </div>
      </a>
    </div>
  </div>

  <!-- Carte 5 -->
  <div id="new-group5" class="new-card-container new-hidden">
    <div class="new-card">
      <a href="link_to_book_E.html">
        <img src="images/pierreBrandao/XLlamuserie.png" class="img-fluid" alt="Couverture de livre">
        <div class="new-card-content">
          <div class="new-card-title">L'ombre du vent</div>
          <div class="new-card-author">Émile Rousseau</div>
          <div class="new-card-price">15,99 €</div>
        </div>
      </a>
    </div>
  </div>

  <!-- Carte 6 -->
  <div id="new-group6" class="new-card-container new-hidden">
    <div class="new-card">
      <a href="link_to_book_F.html">
        <img src="images/pierreBrandao/XLquesavonsnousdumonde.png" class="img-fluid" alt="Couverture de livre">
        <div class="new-card-content">
          <div class="new-card-title">La lumière des ombres</div>
          <div class="new-card-author">Fabien Leclerc</div>
          <div class="new-card-price">17,99 €</div>
        </div>
      </a>
    </div>
  </div>
</div>

<!-- Boutons de navigation -->
<div class="new-navigation d-flex justify-content-center gap-3 mt-4">
  <button id="new-backButton" class="new-button xl-button text-white" disabled>Précédent</button>
  <button id="new-nextGroupButton" class="new-button xl-button text-white">Suivant</button>
</div>

<!-- FIN CARDS XL Mobile -->



 <!-- CARDS XL DESKTOP -->
<div class="d-none d-md-block">
  <!-- Groupe 1 -->
  <div id="new-group1" class="new-card-container">
    <!-- Carte 1 -->
    <div class="new-card">
      <a href="link_to_book_A.html">
        <img src="images/pierreBrandao/XLlamourafleurdame.png" class="img-fluid" alt="Couverture de livre">
        <div class="new-card-content">
          <div class="new-card-title">L'écho des collines</div>
          <div class="new-card-author">Alice Dubois</div>
          <div class="new-card-price">16,99 €</div>
        </div>
      </a>
    </div>
    <!-- Carte 2 -->
    <div class="new-card">
      <a href="link_to_book_B.html">
        <img src="images/pierreBrandao/XLlamuserie.png" class="img-fluid" alt="Couverture de livre">
        <div class="new-card-content">
          <div class="new-card-title">Les murmures du temps</div>
          <div class="new-card-author">Bertrand Martin</div>
          <div class="new-card-price">18,99 €</div>
        </div>
      </a>
    </div>
    <!-- Carte 3 -->
    <div class="new-card">
      <a href="link_to_book_C.html">
        <img src="images/pierreBrandao/XLquesavonsnousdumonde.png" class="img-fluid" alt="Couverture de livre">
        <div class="new-card-content">
          <div class="new-card-title">Le cercle des étoiles</div>
          <div class="new-card-author">Camille Laurent</div>
          <div class="new-card-price">12,99 €</div>
        </div>
      </a>
    </div>
    <!-- Carte 4 -->
    <div class="new-card">
      <a href="link_to_book_D.html">
        <img src="images/pierreBrandao/XLlamourafleurdame.png" class="img-fluid" alt="Couverture de livre">
        <div class="new-card-content">
          <div class="new-card-title">Sous les cyprès</div>
          <div class="new-card-author">Diane Morel</div>
          <div class="new-card-price">14,99 €</div>
        </div>
      </a>
    </div>
    <!-- Carte 5 -->
    <div class="new-card">
      <a href="link_to_book_E.html">
        <img src="images/pierreBrandao/XLlamuserie.png" class="img-fluid" alt="Couverture de livre">
        <div class="new-card-content">
          <div class="new-card-title">L'ombre du vent</div>
          <div class="new-card-author">Émile Rousseau</div>
          <div class="new-card-price">15,99 €</div>
        </div>
      </a>
    </div>
  </div>

  <!-- Groupe 2 -->
  <div id="new-group2" class="new-card-container new-hidden">
    <!-- Carte 6 -->
    <div class="new-card">
      <a href="link_to_book_F.html">
        <img src="images/pierreBrandao/XLquesavonsnousdumonde.png" class="img-fluid" alt="Couverture de livre">
        <div class="new-card-content">
          <div class="new-card-title">La lumière des ombres</div>
          <div class="new-card-author">Fabien Leclerc</div>
          <div class="new-card-price">17,99 €</div>
        </div>
      </a>
    </div>
    <!-- Carte 7 -->
    <div class="new-card">
      <a href="link_to_book_G.html">
        <img src="images/pierreBrandao/XLlamourafleurdame.png" class="img-fluid" alt="Couverture de livre">
        <div class="new-card-content">
          <div class="new-card-title">Les vagues éternelles</div>
          <div class="new-card-author">Gabrielle Blanc</div>
          <div class="new-card-price">13,99 €</div>
        </div>
      </a>
    </div>
    <!-- Carte 8 -->
    <div class="new-card">
      <a href="link_to_book_H.html">
        <img src="images/pierreBrandao/XLlamuserie.png" class="img-fluid" alt="Couverture de livre">
        <div class="new-card-content">
          <div class="new-card-title">Le chant des cendres</div>
          <div class="new-card-author">Henri Dupont</div>
          <div class="new-card-price">14,99 €</div>
        </div>
      </a>
    </div>
    <!-- Carte 9 -->
    <div class="new-card">
      <a href="link_to_book_I.html">
        <img src="images/pierreBrandao/XLquesavonsnousdumonde.png" class="img-fluid" alt="Couverture de livre">
        <div class="new-card-content">
          <div class="new-card-title">La danse des esprits</div>
          <div class="new-card-author">Isabelle Fournier</div>
          <div class="new-card-price">16,99 €</div>
        </div>
      </a>
    </div>
    <!-- Carte 10 -->
    <div class="new-card">
      <a href="link_to_book_J.html">
        <img src="images/pierreBrandao/XLlamuserie.png" class="img-fluid" alt="Couverture de livre ">
        <div class="new-card-content">
          <div class="new-card-title">L'esprit des forêts</div>
          <div class="new-card-author">Jean-Louis Perrin</div>
          <div class="new-card-price">18,99 €</div>
        </div>
      </a>
    </div>
  </div>

  <!-- Groupe 3 -->
  <div id="new-group3" class="new-card-container new-hidden">
    <!-- Carte 11 -->
    <div class="new-card">
      <a href="link_to_book_K.html">
        <img src="images/pierreBrandao/XLlamourafleurdame.png" class="img-fluid" alt="Couverture de livre">
        <div class="new-card-content">
          <div class="new-card-title">Vers l'infini</div>
          <div class="new-card-author">Karen Dupuis</div>
          <div class="new-card-price">16,49 €</div>
        </div>
      </a>
    </div>
    <!-- Carte 12 -->
    <div class="new-card">
      <a href="link_to_book_L.html">
        <img src="images/pierreBrandao/XLlamuserie.png" class="img-fluid" alt="Couverture de livre">
        <div class="new-card-content">
          <div class="new-card-title">Le murmure des vagues</div>
          <div class="new-card-author">Laurent Gauthier</div>
          <div class="new-card-price">14,49 €</div>
        </div>
      </a>
    </div>
    <!-- Carte 13 -->
    <div class="new-card">
      <a href="link_to_book_M.html">
        <img src="images/pierreBrandao/XLquesavonsnousdumonde.png" class="img-fluid" alt="Couverture de livre">
        <div class="new-card-content">
          <div class="new-card-title">Les étoiles du désert</div>
          <div class="new-card-author">Marie Lefèvre</div>
          <div class="new-card-price">18,49 €</div>
        </div>
      </a>
    </div>
    <!-- Carte 14 -->
    <div class="new-card">
      <a href="link_to_book_N.html">
        <img src="images/pierreBrandao/XLlamourafleurdame.png" class="img-fluid" alt="Couverture de livre">
        <div class="new-card-content">
          <div class="new-card-title">Les racines du ciel</div>
          <div class="new-card-author">Nicolas Verdier</div>
          <div class="new-card-price">15,99 €</div>
        </div>
      </a>
    </div>
    <!-- Carte 15 -->
    <div class="new-card">
      <a href="link_to_book_O.html">
        <img src="images/pierreBrandao/XLlamuserie.png" class="img-fluid" alt="Couverture de livre">
        <div class="new-card-content">
          <div class="new-card-title">L'infini en un regard</div>
          <div class="new-card-author">Océane Moreau</div>
          <div class="new-card-price">17,49 €</div>
        </div>
      </a>
    </div>
  </div>
</div>

<!-- Boutons de navigation -->
<div class="new-navigation">
  <button id="new-backButton" class="new-button xl-button text-white" disabled>Précédent</button>
  <button id="new-nextGroupButton" class="new-button xl-button text-white">Suivant</button>
</div>
<!-- FIN CARDS XL -->
